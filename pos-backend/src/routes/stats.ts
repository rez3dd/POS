...
router.get("/overview", async (_req, res, next) => {
  try {
    ...
    // รายได้วันนี้ = รวมยอดของออเดอร์ที่ "ชำระแล้ว" วันนี้
    const paidToday = await prisma.order.findMany({
      where: { status: OrderStatus.PAID, createdAt: { gte: today0 } },
      select: { total: true },
    });
    const revenueToday = paidToday.reduce((s, o) => s + (o.total ?? 0), 0);
    const ordersToday = paidToday.length;
    const avgTicket = ordersToday ? Math.round(revenueToday / ordersToday) : 0;

    // ออเดอร์กำลังดำเนินการ = ที่ยัง "ไม่ชำระ"
...
router.get("/daily", async (req, res, next) => {
  try {
    ...
    const orders = await prisma.order.findMany({
      where: { status: OrderStatus.PAID, createdAt: { gte: from } },
...
router.get("/payments", async (_req, res, next) => {
  try {
    // ยังไม่ทำ Breakdown ตาม method → ส่งว่างไว้ก่อน หรือจะนับจาก method ก็ได้
    res.json([]);
  } catch (e) {
    next(e);
  }
...
router.get("/top-dishes", async (req, res, next) => {
  try {
    ...
    const agg = new Map<number, { menuId: number; qty: number; revenue: number }>();
    for (const o of paidOrders) {
      for (const it of o.items) {
        const cur = agg.get(it.menuId) || { menuId: it.menuId, qty: 0, revenue: 0 };
        cur.qty += it.qty;
        cur.revenue += it.qty * (it.price ?? 0); // ป้องกัน price เป็น null

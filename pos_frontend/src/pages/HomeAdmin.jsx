...
const LIMIT = 7;          // ⬅️ แสดงสูงสุด 7 รายการ
const POLL_MS = 15000;    // ⬅️ รีเฟรชทุก 15 วิ
...
export default function HomeAdmin() {
  ...
  async function loadAll() {
    const [ov, ro, td] = await Promise.all([
      getStatsOverview(),
      listOrders({ limit: LIMIT, sort: "desc" }), // ⬅️ ขอมาแค่ 7
      getTopDishes(LIMIT),                        // ⬅️ ขอมาแค่ 7
    ]);
    setOverview(ov || {});
    const r = Array.isArray(ro) ? ro : ro?.data || [];
    const p = Array.isArray(td) ? td : td?.data || [];
    // จัดเรียง/ตัดความยาว กันเหนียว
    setRecent(
      r.sort((a, b) => new Date(b.createdAt) - new Date(a.createdAt)).slice(0, LIMIT)
    );
    setPopular(
      p.sort((a, b) => (b.orders ?? 0) - (a.orders ?? 0)).slice(0, LIMIT)
    );
  }
  ...
  useEffect(() => {
    ...
    (async () => {
      try {
        ...
        console.error("HomeAdmin load error:", e);
      } finally {
        ...
        <div className="flex items-end justify-between mb-6">
          <div>
            <h1 className="text-2xl font-extrabold text-white">
              สวัสดีตอนเช้า, <span className="text-yellow-400">Admin</span>
            </h1>
